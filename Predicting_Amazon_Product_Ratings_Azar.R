...
mydata = read_excel("Amazon_dataset - Copy.xlsx")
head(mydata)
...
# Which lambda gives me this value?
which.min(testing_loss)
lambda_values[which.min(testing_loss)]

#Computing MSE and MAE
MSE_LASSO_without_CV = min(testing_loss)
MaE_LASSO_without_CV= min (testing_loss_mae)
MSE_LASSO_without_CV
MSE_LASSO_without_CV

lambda = lambda_values[which.min(testing_loss)]
model = glmnet(trainingX, trainingy, family = 'gaussian', alpha = 1, lambda = lambda)
...
which.min(MSE_LASSO_with_CV)
lambda_values[which.min(MSE_LASSO_with_CV)]
min(MSE_LASSO_with_CV)
...
lambda_values[which.min(MSE_kernel_smoothing)]
which.min(MSE_kernel_smoothing)
which.min(MAE_kernel_smoothing)
min(MSE_kernel_smoothing)

MSE_kernel_smoothing
